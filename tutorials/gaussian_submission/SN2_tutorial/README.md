...
- Download the [submission script](https://github.com/Trujillo-Group/trujillo_group_resources/tree/main/scripts/submission/CSF3-4): `sub_csf3` and place this in your CSF bin folder
...
- Download [submission script](https://github.com/Trujillo-Group/trujillo_group_resources/tree/main/scripts/submission/CSF3-4): `sub_csf4` and place this in your CSF bin folder
- Make the script executable with
...
1. **_GaussView 6_** is used to create a model of the molecule(s) you wish to study and to create the input files for **_Gaussian 16_**.

2. The files of these molecules are then edited to include information necessary for **_Gaussian 16_** to read.

3. The files are submitted to **_CSF_** which runs the calculations using **_Gaussian 16_**.

## 1. GaussView

### Open GaussView on CSF

(**_Does not work for Mac Users_**)

1. Open a terminal and connect to CSF3: `ssh -X csf3`
2. Open an interactive session on a computer node: `srun -p interactive -t 0-1 --pty bash​`
3. Load the Gaussian16 module: `module load apps/binapps/gaussian/g16c01_em64t_detectcpu`
...
1. Select the _Element Fragment_ tool (<span style="color:cyan">**blue**</span>)
2. Click in the _Atom Selection_ tool (<span style="color:red">**red**</span>) to open the periodic table.
3. Select the _SP3_ tetravalent oxygen (<span style="color:green">**green**</span>).
...
5.  Select the _Delete Atom_ tool
...
_Don't forget to exit your interactive session via `exit`._
...
- Run `squeue` and check if it is pending (**PD**) or running (**R**).
...
(In case you've forgotten any names & need help: Cris, Tim, James, Julia)
